...
pub fn event_to_wire(event: &TcpDeviceEvent) -> (&'static str, Option<Value>) {
    match event {
        TcpDeviceEvent::Connection(value) => ("connection", Some(Value::Bool(*value))),
        TcpDeviceEvent::Data(value) => ("data", Some(Value::String(value.clone()))),
    }
}

pub fn dispatch_event(handler: &SharedEventHandler, name: &str, event: &TcpDeviceEvent) {
